...
@R4 Value of i
@R5 Address of i
@R10 counter j starting at 1
@R12 Result of part 1 of arithmetic operation
SUBROUTINE:
	...
	SUB R7, R0, #1 @Loop counter where k = N-1
...
@R4 Counter i
@R5 Address of x_array
@R6 x_b term
@R7 Loop counter k which starts at N-1
@R9 Address of y_array, y_a term
@R10 Counter j
@R11 Counter storing value of i-1
LOOP:

	CMP R7, #0 @Compare loop counter with 0
	BLT EXIT @Exits loop if less than 0
	...
	SDIV R6, R6, R11 @Divides result by a[0]
	ADD R12, R12, R6 @Add the results together

	SUB R7, R7, #1 @Decrement k counter
...
@R7 Address of x_array
@R8 Address of y_array
